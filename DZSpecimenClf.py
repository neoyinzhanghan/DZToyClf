...
class Attn(nn.Module):
    """Attention mechanism that computes scaled dot product attention.
    
    Args:
        head_dim (int): Dimension size of each attention head
        use_flash_attention (bool): Whether to use PyTorch's flash attention optimization
    """
...
class MultiHeadAttentionClassifier(nn.Module):
    """Multi-head attention classifier that processes sequences using self-attention.
    
    This module implements a multi-head attention classifier that first projects the input
    into query, key and value spaces, applies scaled dot-product attention across multiple
    heads in parallel, and then uses a learnable class token for final classification.
    The module can optionally use PyTorch's flash attention optimization.

    Args:
        d_model (int): Size of input feature dimension. Must be divisible by num_heads.
            Default: 2048
        num_heads (int): Number of attention heads to use in parallel. Default: 8
        use_flash_attention (bool): Whether to use PyTorch's flash attention optimization.
            Default: True
    """
    def __init__(
...
class DZSpecimenClf(nn.Module):
    """A neural network module for classifying specimens using a dual-zoom approach.

    This module combines a ResNeXt50 backbone for feature extraction with an attention-based
    classifier. It processes both a top-view image and search view indexibles to make classifications.

    Args:
        N (int): Number of patches to extract from the search view
        patch_size (int, optional): Size of patches to extract. Defaults to 224.
...
class DZSpecimenClf_SamplingGradientTester(nn.Module):
    """A neural network module for classifying specimens using a dual-zoom approach.
    ...
    def __init__(self, N, patch_size=224, num_classes=2, x_max=224 * 16, y_max=224 * 16):
        ...
        self.patch_size = patch_size
        self.num_classes = num_classes
        self.xy = torch.rand(N, 2) * torch.tensor([x_max - patch_size, y_max - patch_size]) + torch.tensor([patch_size//2, patch_size//2])
        
        # make self.xy a trainable tensor
    ...
    def forward(self, topview_image_tensor, search_view_indexibles):
        ...
        # Continue with x_scaled instead of x
        x = differentiable_crop_2d_batch(search_view_indexibles, locations)
